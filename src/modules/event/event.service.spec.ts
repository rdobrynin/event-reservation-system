...
describe('EventService', () => {
    ...
    const mockEventWithBookings = {
        ...
        name: 'test_event_2',
        totalSeats: 10,
        booking: [{ id: '1943b2b1-5b9a-4570-8bfb-d0303856eed5' }, { id: 'f33e0cc3-e6f5-457e-84b7-c3c9845b5128' }],
        toDto: jest.fn().mockReturnValue({
            id: 'c635982c-297a-4ac1-8b6f-94ccae45465c',
            name: 'test_event_2',
            totalSeats: 10,
    ...
    describe('create', () => {
        ...
        it('should create an event successfully when seats are within limit', async () => {
            ...
            expect(mockRepository.save).toHaveBeenCalledWith(expect.objectContaining(createEventDto));
        ...
        it('should throw EventExceedPlaceException when seats exceed limit', async () => {
            ...
            // Act & Assert
            await expect(service.create(largeEventDto)).rejects.toThrow(EventExceedPlaceException);
    ...
    describe('getEventForReserve', () => {
        ...
        it('should throw EventExceedPlaceException when no seats available', async () => {
            ...
            // Act & Assert
            await expect(service.getEventForReserve(createReserveDto)).rejects.toThrow(EventExceedPlaceException);
        });
    ...
    describe('getOne', () => {
        it('should return event DTO when found', async () => {
            ...
            // Assert
            expect(result).toEqual(mockEvent.toDto());
            expect(queryBuilder.where).toHaveBeenCalledWith('event.id = :id', { id: '1' });
        });
        ...
        it('should throw EventNotFoundException when event not found', async () => {
            ...
            // Act & Assert
            await expect(service.getOne('999')).rejects.toThrow(EventNotFoundException);
        });
    });
});
